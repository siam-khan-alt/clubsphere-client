                ...
                <div className="bg-card rounded-2xl border-standard shadow-xl overflow-hidden">
                    <div className="p-8 md:p-10">
                        ...
                        <div className="text-center mb-8">
                            <h2 className="!text-4xl !mb-2 leading-tight">Join <span className="not-italic">ClubSphere</span></h2>
                            <p className="text-text-body font-medium opacity-70 text-sm">Create your account to get started</p>
                        </div>

                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                            {/* Full Name */}
                            <div className="flex flex-col gap-2">
                                <label className="text-[11px] font-black uppercase tracking-widest text-text-heading ml-1 opacity-60">Full Name</label>
                                <div className="relative">
                                    <span className="absolute inset-y-0 left-4 flex items-center text-text-body/40"><FiUser size={18}/></span>
                                    <input
                                        type="text"
                                        placeholder="Your Name"
                                        className={`input-field-custom w-full pl-12 ${errors.name ? 'border-red-500 focus:border-red-500' : ''}`}
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-[11px] font-black uppercase tracking-widest text-text-heading ml-1 opacity-60">Profile Image</label>
                                <div className="relative group">
                                    <input
                                        ...
                                        accept="image/*"
                                    />
                                    <div className={`p-5 rounded-2xl border-2 border-dashed transition-all flex flex-col items-center justify-center gap-2 
                                        ${errors.image ? 'border-red-500 bg-red-500/5' : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 group-hover:border-primary/50 group-hover:bg-primary/5'}`}>
                                        <FiUploadCloud className={`text-2xl ${errors.image ? 'text-red-500' : 'text-primary animate-pulse'}`} />
                                ...
                                {errors.image && <p className="text-red-500 text-[10px] font-bold mt-1 uppercase text-center">{errors.image.message}</p>}
                            </div>

                            {/* Email Address */}
                            <div className="flex flex-col gap-2">
                                <label className="text-[11px] font-black uppercase tracking-widest text-text-heading ml-1 opacity-60">Email Address</label>
                                <div className="relative">
                                    <span className="absolute inset-y-0 left-4 flex items-center text-text-body/40"><FiMail size={18}/></span>
                                    <input
                                        type="email"
                                        placeholder="name@example.com"
                                        className={`input-field-custom w-full pl-12 ${errors.email ? 'border-red-500 focus:border-red-500' : ''}`}
                            ...
                            {/* Password */}
                            <div className="flex flex-col gap-2">
                                <label className="text-[11px] font-black uppercase tracking-widest text-text-heading ml-1 opacity-60">Password</label>
                                <div className="relative">
                                    <span className="absolute inset-y-0 left-4 flex items-center text-text-body/40"><FiLock size={18}/></span>
                                    <input
                                        type={showPassword ? 'text' : 'password'}
                                        placeholder="••••••••"
                                        className={`input-field-custom w-full pl-12 pr-12 ${errors.password ? 'border-red-500 focus:border-red-500' : ''}`}
                                        {...register("password", { 
                        ...
                        <div className="relative my-8 text-center">
                            ...
                            <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-card px-4 text-[10px] font-black uppercase tracking-[0.3em] text-text-body/40">OR</span>
                        </div>
                        ...
                        <p className="text-center mt-10 text-sm font-bold text-text-body/60">
                            ...
                            <Link to="/login" className="text-primary hover:underline underline-offset-4 decoration-2 transition-all">
                                Login here
                            </Link>
                        </p>
                    </div>
