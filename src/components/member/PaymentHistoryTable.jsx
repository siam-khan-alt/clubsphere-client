    ...
    if (payments.length === 0) {
        return (
            <div className="text-center py-10 bg-white rounded-lg shadow-md">
                <p className="text-gray-500 text-lg">You have no payment records yet.</p>
            </div>
        );
    }
            ...
            <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-blue-50">
                    <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Club/Event</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {payments.map((payment) => (
                        <tr key={payment._id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {payment.clubName || payment.eventName || 'N/A'} 
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                    payment.status === 'paid' || payment.paymentStatus === 'succeeded' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                }`}>
